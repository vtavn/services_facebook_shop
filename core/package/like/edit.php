...
<?php 
        $req = mysqli_query($vtasystem, "SELECT * FROM `package_vip` WHERE `id` = '".$_GET[edit]."'"); 
  ...
  if (isset($_POST['submit'])) { 
        ...
        $name = $_POST['name']; 
        mysqli_query($vtasystem, "UPDATE package_vip SET `name` = '$name', `giatien` = '$giatien', `limit_post`= '$limitpost', `limit_like`= '$limitlike' WHERE `id` = '".$_GET[edit]."'"); 
        echo "<script>alert('Thành công');window.location='/index.php?vip=list_package_like';</script>"; 
...
<form action="" method="POST">
	...
	<div class="form-group">
	  <label for="usr">Limit Cảm Xúc:</label>
	  <input type="number" class="form-control" name="limitlike" value="<?php echo $vtadz['limit_like']; ?>">
	</div>
 	<div class="form-group">
	...
	<button class="btn btn-success pull-right"><a href="/index.php?vip=list_package_like" style="color:white">Quay lại</a></button>
