...
$app = json_decode(file_get_contents("https://graph.facebook.com/app?access_token=".$token),true);   
    if(empty($app['id'])) 
        { 
            echo '<meta http-equiv=refresh content="0; URL=/index.php?vip=add_vipbot">'; 
...
die('<script>alert("Không bug được đâu nhé! Gian lận không tốt đâu!");</script>'); 
        ...
        $checkvips = "SELECT * FROM `vip_bot` WHERE `idfb`= '$idfb' LIMIT 1";
...
mysqli_query($vtasystem, "INSERT INTO `vip_bot` SET `idfb`='$idfb', `name`='$name', `token`='$token', `camxuc`='$camxuc', `name_buy`='".$getuser['username']."', `id_buy`='".$getuser['id']."', `time`='$timecx',`gioihan`='$gioihan', `status`='0', `cookie`='".$_SESSION['cookie']."', `solanchay`='0'");

    //up log 
        $timess = time(); 
        $content = "<b>" .$getuser['username']. "</b> vừa thêm VIP BOT cho ID <b>$idfb</b>. Thời hạn <b>1</b> tháng, Loại CX: <b>$camxuc</b>, tổng thanh toán <b>" . number_format($vndbot) . " VNĐ </b>"; 
...
<div class="col-lg-6"> 
 <div class="panel panel-default"> 
                   <div class="panel-heading"> 
                             <b><i class="fa fa-gears"></i> Cài Đặt Bot Cảm Xúc Pro</b> 
                        </div> 
