...
echo '<pre>'; 
$vip = mysqli_query($vtasystem, "SELECT * FROM vip_like WHERE status = 'checked' AND id_buy != 812 ORDER BY RAND() LIMIT 25"); 
                    ...
                    if ($likeConLai < $speedLike) { 
                        if ($likeConLai <= 0) { 
                            ...
                            $post_data = array( 
                                'time_delay' => 500, 
                                'id' => $sttID, 
                                'typeReact' => explode('|', $row['camxuc']),// quên trên này :3 
                                'access_token' => $TOKEN 
                            ); 
                        } 
                    } else { 
